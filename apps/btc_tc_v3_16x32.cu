...
int main(int argc, char** argv)
{
    ...
    std::printf("\n--------------btc 16x32 v3 (discrete K)----------------\n");
    {
        timer.start();
        btc::BCSR_16x32_Device d_bcsr;
        btc::convert_coo_to_bcsr_16x32_gpu(d_bcsr, A_coo);
        timer.stop();
        std::printf("[Converting to BCSR 16x32] time: %lf ms\n", timer.elapsed());
        std::printf("  Num Blocks: %u\n", d_bcsr.num_blocks);

        timer.start();
        auto count = btc::count_triangles_16x32_v3(d_bcsr);
        timer.stop();
        std::printf("[Counting Triangles (16x32 v3 discrete K)] time: %f ms\n", timer.elapsed());
