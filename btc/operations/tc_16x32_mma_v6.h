...
// V6 kernel: SR config — Shuffle for sparsity (half-warp), Redux for final reduction (full-warp)
__global__ void kernel_16x32_mma_v6_no_binsearch(
    ...
    unsigned long long* __restrict__ result)
{
    ...
    // Step 1: Sparsity calculation using shuffle reduction
    // Only 16 lanes contribute (half-warp), shuffle avoids wasting the idle lanes
    // that redux.sync would force to participate. ~3.75% faster on 16×32.
    int local_nnz = 0;
    if (lane_id < 16) {
        local_nnz = __popc(S_ptr[lane_id]);
    }

    // Warp-level reduction via shuffle tree (better for half-warp participation)
    #pragma unroll
...
unsigned long long count_triangles_16x32_v6(BCSR_16x32_Device& d_bcsr, float* kernel_ms = nullptr) {
    d_bcsr.reset_result();
    ...
    kernel_16x32_mma_v6_no_binsearch<<<num_blocks, threads_per_block>>>(
