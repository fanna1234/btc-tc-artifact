...
// 16x128 SR: Shuffle for sparsity, Redux for final
__global__ void kernel_16x128_SR(int n, int n_row_blocks, const uint32_t* __restrict__ indptr,
    ...
    const uint32_t* __restrict__ row_indices, uint32_t num_sample_blocks, unsigned long long* __restrict__ result)
{
    ...
    asm volatile("redux.sync.add.u32 %0, %1, 0xFFFFFFFF;" : "=r"(reduced_sum) : "r"(total_sum));
    if (lane_id == 0) atomicAdd(result, (unsigned long long)reduced_sum);
}

unsigned long long count_triangles_16x128_SR(btc::BCSR_16x128_Device& d_bcsr, float* kernel_ms = nullptr) {
    d_bcsr.reset_result();
    ...
    kernel_16x128_SR<<<blocks, threads>>>(d_bcsr.n, d_bcsr.n_row_blocks, d_bcsr.indptr, d_bcsr.indices, d_bcsr.blocks, d_bcsr.row_indices, d_bcsr.num_blocks, d_bcsr.result);
