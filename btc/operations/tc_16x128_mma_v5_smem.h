...
// V5_SMEM kernel: V5 + Shared Memory caching for row I indices
// Optimization: Cache row I's indices in shared memory to reduce repeated global memory reads
__global__ void kernel_16x128_mma_v5_smem(
    ...
    unsigned long long* __restrict__ result)
{
    ...
    constexpr int MAX_CACHED_BLOCKS = 256;  // Max blocks per row to cache in shared memory

    // Shared memory for caching row I's indices (per warp)
    __shared__ uint32_t s_indices_I[4][MAX_CACHED_BLOCKS];  // 4 warps per block
    __shared__ int s_num_blocks_I[4];

    int warp_id_global = (blockIdx.x * blockDim.x + threadIdx.x) / WARP_SIZE;
    int warp_id_local = threadIdx.x / WARP_SIZE;  // 0-3 within block
    ...
    // Load row I's indices into shared memory (warp-collaborative)
    int num_blocks_I = (int)(end_I - start_I);
    bool use_smem = (num_blocks_I <= MAX_CACHED_BLOCKS);

    if (use_smem) {
        // Warp collaboratively loads indices into shared memory
        for (int i = lane_id; i < num_blocks_I; i += WARP_SIZE) {
            s_indices_I[warp_id_local][i] = indices[start_I + i];
        }
        if (lane_id == 0) {
            s_num_blocks_I[warp_id_local] = num_blocks_I;
        }
        __syncwarp();
    }
    ...
    if (use_dense) {
        ...
        for (int iter = 0; iter < 8; iter++) {
            ...
            int ptr_A = 0, ptr_B = j_start;  // ptr_A now indexes into s_indices_I
            while (ptr_A < num_blocks_I && ptr_B < j_end) {
                uint32_t K_A = use_smem ? s_indices_I[warp_id_local][ptr_A] : indices[start_I + ptr_A];
                ...
                if (K_A < K_B) {
                    ...
                    int actual_ptr_A = start_I + ptr_A;
                    const uint32_t* A_blk = blocks + (size_t)actual_ptr_A * SIZE_U32;
...
unsigned long long count_triangles_16x128_v5_smem(BCSR_16x128_Device& d_bcsr, float* kernel_ms = nullptr) {
    d_bcsr.reset_result();
    ...
    kernel_16x128_mma_v5_smem<<<num_blocks, threads_per_block>>>(
