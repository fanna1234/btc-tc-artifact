...
// V5-shfl: Same as V5 but replaces redux.sync with __shfl_down_sync + broadcast
// For ablation study: measuring redux.sync vs shuffle reduction overhead
__global__ void kernel_16x128_mma_v5_shfl(
    ...
    unsigned long long* __restrict__ result)
{
    ...
    // Step 1: Calculate sparsity using SHUFFLE (ablation: replacing redux.sync)
    int local_nnz = 0;
    for (int i = lane_id; i < SIZE_U32; i += WARP_SIZE) {
        local_nnz += __popc(S_ptr[i]);
    }

    // Warp-level reduction using __shfl_down_sync (tree reduction)
    #pragma unroll
            ...
            while (ptr_A < (int)end_I && ptr_B < j_end) {
                ...
                while (mask != 0) {
                    ...
                    for (int u = 0; u < COLS_U32; u++) {
                        sum += __popc(A_blk[my_row * COLS_U32 + u] & B_blk[col * COLS_U32 + u]);
                    }
                    total_sum += sum;
                    mask &= (mask - 1);
                }
...
unsigned long long count_triangles_16x128_v5_shfl(BCSR_16x128_Device& d_bcsr, float* kernel_ms = nullptr) {
    d_bcsr.reset_result();
    ...
    kernel_16x128_mma_v5_shfl<<<num_blocks, threads_per_block>>>(
