...
//------------------------------------------------------------------------------
// Customized atomic wrapper using GCC __atomic builtins
// Compatible with Boost >= 1.83 (no trivially-copyable restriction from boost::atomic)
// - Uses acquire/release/acq_rel as default memory orders
...
union atomix {
  ...
  atomix() {
    T z = T();
    __atomic_store(&raw, &z, __ATOMIC_RELAXED);
  }
  atomix(const atomix<T>& x) {
    T v = static_cast<T>(x);
    __atomic_store(&raw, &v, __ATOMIC_RELAXED);
  }
  atomix(T x) {
    __atomic_store(&raw, &x, __ATOMIC_RELAXED);
  }

  operator T() const {
    T result;
    __atomic_load(&raw, &result, __ATOMIC_ACQUIRE);
    return result;
  }

  // fetch_add: only valid for integral types (size_t, uint32_t, etc.)
  template<typename U = T, typename = std::enable_if_t<std::is_integral<U>::value>>
  T fetch_add(T x) {
    return __atomic_fetch_add(&raw, x, __ATOMIC_ACQ_REL);
  }

  // fetch_add for float: CAS loop
  template<typename U = T, typename std::enable_if_t<std::is_floating_point<U>::value, int> = 0>
  T fetch_add(T x) {
    T old_val;
    __atomic_load(&raw, &old_val, __ATOMIC_ACQUIRE);
    T new_val;
    do {
      new_val = old_val + x;
    } while (!__atomic_compare_exchange(&raw, &old_val, &new_val, true,
  ...
  T exchange(T x) {
    T result;
    __atomic_exchange(&raw, &x, &result, __ATOMIC_ACQ_REL);
    return result;
  }

  bool compare_exchange_weak(T& exp, T x) {
    return __atomic_compare_exchange(&raw, &exp, &x, true,
                                     __ATOMIC_ACQ_REL, __ATOMIC_ACQUIRE);
  }

  atomix& operator=(const atomix<T>& x) {
    T v = static_cast<T>(x);
    __atomic_store(&raw, &v, __ATOMIC_RELEASE);
    return *this;
  }
  atomix& operator=(T x) {
    __atomic_store(&raw, &x, __ATOMIC_RELEASE);
    return *this;
  }
