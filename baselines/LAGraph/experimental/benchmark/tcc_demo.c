...
int main (int argc, char **argv)
{
    ...
    GrB_free (&(G->A)) ;
    G->A = A ;
    A = NULL ;  // G now owns the matrix; avoid accidental double-free
    ...
    GRB_TRY (GrB_free (&c)) ;
    // Print triangles as an integer (uint64_t). Using %g loses precision for large counts.
    printf ("warmup time %g sec, # triangles: %" PRIu64 "\n", tt, ntri) ;
