...
// SuiteSparse:GraphBLAS internal hook used by tc_gpu_demo to force GPU on/off.
// This is not a standard GraphBLAS API.
...
static inline int demo_init (bool burble)
{
    ...
    #if defined ( GRAPHBLAS_HAS_CUDA ) && !defined ( EXPERIMENTAL_GPU )
    // Ensure CPU demos never offload to the GPU.  GraphBLAS GPU kernels assume
    // unified-memory allocations, which are only guaranteed in *_GPU modes.
    GB_Global_hack_set (2, 2) ; // never use the GPU
    #endif

    // Ensure GraphBLAS CUDA JIT picks the full CUDA toolkit nvcc (not the tiny
    // /usr/bin/nvcc wrapper), otherwise nvcc may fail to find internal tools
    // like cicc at runtime.
    setenv ("PATH", "/usr/local/cuda/bin:/usr/local/cuda/nvvm/bin:/usr/bin:/bin",
        /*overwrite*/ 1) ;

    // Prefer the direct compile path for GraphBLAS JIT kernels.  In some
    // environments the CMake-based JIT can pick up problematic link flags
    // (such as a static libpthread), causing GxB_JIT_ERROR at runtime.
    (void) GxB_Global_Option_set_INT32 (GxB_JIT_USE_CMAKE, 0) ;

    // Optional: make JIT failures easier to debug (can be inspected after run).
    (void) GxB_Global_Option_set_CHAR (GxB_JIT_ERROR_LOG,
        "/tmp/graphblas_jit_error.log") ;
