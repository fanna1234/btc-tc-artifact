...
static inline cudaError_t GB_cudaMemPrefetchAsync
(
    const void *ptr,
    size_t bytes,
    int device,
...
)
{
    ...
    {
        (void) GB_cudaMemPrefetchAsync (A->p, (anvec+1) * psize,
            device, stream) ;
    }

    if (A->h != NULL && (which & GB_PREFETCH_H))
    {
        (void) GB_cudaMemPrefetchAsync (A->h, anvec * jsize,
            device, stream) ;
    }
    ...
    {
        (void) GB_cudaMemPrefetchAsync (A->b, anz * sizeof (int8_t),
            device, stream) ;
    }

    if (A->i != NULL && (which & GB_PREFETCH_I))
    {
        (void) GB_cudaMemPrefetchAsync (A->i, anz * isize,
            device, stream) ;
    }

    if (A->x != NULL && (which & GB_PREFETCH_X))
    {
        (void) GB_cudaMemPrefetchAsync (A->x, (A->iso ? 1:anz) * A->type->size,
            device, stream) ;
    }
