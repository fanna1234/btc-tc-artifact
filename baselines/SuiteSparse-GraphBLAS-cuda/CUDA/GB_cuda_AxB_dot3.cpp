...
static inline cudaError_t GB_cudaMemAdvise
(
    const void *ptr,
    size_t bytes,
...
)
{
    ...
    (void) GB_cudaMemAdvise (C->p, (cnvec+1) * psize,
        cudaMemAdviseSetPreferredLocation, device) ;
    if (M_is_hyper)
    { 
        (void) GB_cudaMemAdvise (C->h, cnvec * jsize,
            cudaMemAdviseSetPreferredLocation, device) ;
    }
    (void) GB_cudaMemAdvise (C->i, (cnz+1) * isize,
        cudaMemAdviseSetPreferredLocation, device) ;
    if (!C_iso)
    {
        (void) GB_cudaMemAdvise (C->x, (cnz+1) * C->type->size ,
            cudaMemAdviseSetPreferredLocation, device) ;
    }
