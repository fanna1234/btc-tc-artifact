...
/**
 * @brief Record the id before transformation, which is used to generate d_idMapArr and part the degree
 */
__global__ void record_id_and_part_degree(int edge_count, int vertex_count, int *d_idArr, int *d_degreeArr);
...
/**
 * @brief Calculate the out-degree of src
 */
...
void compute()
{
    ...
    for (int k = 0; k < iteration; k++)
    {
        ...
        record_id_and_part_degree<<<vertex_grid_size, block_size>>>(edge_count, vertex_count, d_idArr, d_degreeArr);
        HRR(cudaDeviceSynchronize());
