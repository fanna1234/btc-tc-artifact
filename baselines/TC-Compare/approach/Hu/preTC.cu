...
#pragma omp parallen for
	...
	// for (int i = 0; i < bucketNum; i++)
	// {
	// 	for (int j = 0; j < edgeBucket[i].esize; j++)
	// 	{
	// 		int src = edgeBucket[i].Edges[j].src;
	// 		int dst = edgeBucket[i].Edges[j].dst;
	// 		edge curEdge;
	// 		curEdge.src = src;
	// 		curEdge.dst = dst;
	...
	// ofstream fWrite_normal("normal", ios::out | ios::binary);
	// fWrite_normal.write((char *)&edgeRow[0], sizeof(int) * edgeNum);
	// fWrite_normal.close();
