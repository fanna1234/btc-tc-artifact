...
bool preProcess(const char *fileName, unsigned int &_edgeNum, unsigned &_nodeNum)
{
	...
	for (unsigned int i = 0; i < edgeNum; i++)
	{
		if (Edges[i].src > maxNodeID)
			maxNodeID = Edges[i].src;
		if (Edges[i].dst > maxNodeID)
			maxNodeID = Edges[i].dst;
	}

	int *degrees = new int[maxNodeID + 1];
	memset(degrees, 0, sizeof(int) * (maxNodeID + 1));
	for (unsigned int i = 0; i < edgeNum; i++)
	{
		degrees[Edges[i].src] += 1;
		degrees[Edges[i].dst] += 1;
	}
// // unsigned int maxNodeID = 0;
#pragma omp parallel for
	for (unsigned int i = 0; i < edgeNum; i++)
	{
		int srcDegree = degrees[Edges[i].src];
		int dstDegree = degrees[Edges[i].dst];
		if (srcDegree > dstDegree || (srcDegree == dstDegree && Edges[i].src > Edges[i].dst))
		{
			int tmpValue = Edges[i].dst;
			Edges[i].dst = Edges[i].src;
			Edges[i].src = tmpValue;
	...
	for (int i = Edges[edgeNum - 1].src + 1; i <= nodeNum; i++)
	{
	...
	// cout << "xxxxxxxxxxxx " << _nodeNum << "  " << edgeNum << endl;
#pragma omp parallel for
	for (int i = 0; i < _nodeNum; i++)
	{
		...
		{
			if (edgeOffset[i] + j > edgeNum)
			{
				// cout << edgeOffset[i] + j << " " << edgeOffset[i] << " " << j << " " << edgeNum << endl;
			}
