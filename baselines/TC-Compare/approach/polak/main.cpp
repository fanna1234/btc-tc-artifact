...
int main(int argc, char *argv[])
{
  ...
  int iterator_count = atoi(argv[3]);
  ...
  cout << "Number of nodes: " << num_nodes
       << ", number of edges: " << num_edges / 2 << endl;

  vector<uint64_t> results;

#define TEST_ALGORITHM(algorithm, ...)         \
  do                                           \
  {                                            \
  ...
  PreInitGpuContext(device);
  t->Done("Preinitialize context for device 0");
  TEST_ALGORITHM(GpuForward, edges,iterator_count);
  // for (int i = 0; i < 4; ++i)
  //   PreInitGpuContext(i);
