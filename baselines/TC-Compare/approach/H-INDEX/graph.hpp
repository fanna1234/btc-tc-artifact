	...
	string jsonfile) //,
{
	...
	string s_head = jsonfile + "/source.bin";
	...
	// fread(edge_list, sizeof(vertex_t), edge_count * 2, pFile2);
	// fclose(pFile2);

	edge_list_count = edge_count * 2;
	edge_list = (vertex_t *)malloc(edge_list_count * sizeof(vertex_t));
	for (index_t i = 0; i < edge_count; i++)
	{
		edge_list[i * 2] = adj_list[i];
		edge_list[i * 2 + 1] = head_list[i];
	}
	free(head_list);
